 ...
 */
export default class LombardiCalc extends BaseCalc
{
    /**
     * Constructor.
     * 
    ...
    calcMult()
    {
        if (!this.checkOpts(['reps'])) {
            throw new WtCalculatorError(`Lombardi calculation does not have required options (multi).`);
        }
    ...
    calc()
    {
        ...
        if (!this.checkOpts(['liftedWeight'])) {
            throw new WtCalculatorError(`Lombardi calculation does not have required options.`);
        }
