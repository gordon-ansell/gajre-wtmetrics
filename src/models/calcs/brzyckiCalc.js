...
export default class BrzyckiCalc extends BaseCalc
{
    ...
    calcMult()
    {
        if (!this.checkOpts(['reps'])) {
            throw new WtCalculatorError(`Brzycki calculation does not have required options (multi).`);
        }
    ...
    calc()
    {
        ...
        if (!this.checkOpts(['liftedWeight'])) {
            throw new WtCalculatorError(`Brzycki calculation does not have required options.`);
        }
